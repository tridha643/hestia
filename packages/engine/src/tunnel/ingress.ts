...
/**
 * Base rules first (the user's primary worktree), then per-worktree dynamic
 * rules, then the mandatory catch-all. Host is rewritten to the origin by
 * default — vite/next dev servers reject foreign Hosts (allowedHosts /
 * allowedDevOrigins) — unless the exposure opted out.
 */
 ...
 */
function assertDisjoint(base: IngressRule[], dynamic: DynamicRule[]): void {
