...
export async function composeUp(ctx: ComposeCtx): Promise<void> {
  await docker(ctx, ["up", "-d"]);
}
...
export async function composePs(ctx: ComposeCtx): Promise<PsService[]> {
  const { stdout } = await docker(ctx, ["ps", "--format", "json"], 30_000);
...
export function publishedPortFor(
  ...
  if (containerPort !== undefined) {
    const match = pubs.find((p) => p.TargetPort === containerPort);
